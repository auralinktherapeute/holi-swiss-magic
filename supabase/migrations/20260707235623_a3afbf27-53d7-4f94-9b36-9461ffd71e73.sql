...
-- Colonne + trigger (to_tsvector avec regconfig est STABLE => impossible en generated)
ALTER TABLE public.therapists ADD COLUMN IF NOT EXISTS search_tokens tsvector;

CREATE OR REPLACE FUNCTION public.therapists_refresh_search_tokens()
...
BEGIN
  NEW.search_tokens := to_tsvector('simple',
    public.immutable_unaccent(
      ...
      coalesce(NEW.bio,'')        || ' ' ||
      array_to_string(coalesce(NEW.specialties,'{}'::text[]),' ') || ' ' ||
      array_to_string(coalesce(NEW.approaches,'{}'::text[]),' ')  || ' ' ||
      coalesce(NEW.slug,'')
    )
  );
  RETURN NEW;
...
DROP TRIGGER IF EXISTS therapists_search_tokens_trg ON public.therapists;
CREATE TRIGGER therapists_search_tokens_trg
  BEFORE INSERT OR UPDATE OF first_name,last_name,title,city,canton,short_bio,bio,specialties,approaches,slug
  ON public.therapists FOR EACH ROW EXECUTE FUNCTION public.therapists_refresh_search_tokens();

-- Backfill existant
UPDATE public.therapists SET updated_at = updated_at;

CREATE INDEX IF NOT EXISTS therapists_search_tokens_gin
  ON public.therapists USING gin (search_tokens);
...
BEGIN
  ...
  FOREACH v_tok IN ARRAY v_tokens LOOP
    ...
    IF NOT v_has_geo THEN
      ...
      LIMIT 1;
      IF FOUND THEN v_has_geo := true; v_found := true; END IF;
    END IF;
  ...
  base AS (
    ...
    WHERE t.status = 'active'
      ...
      AND (array_length(v_spec_ids,1) IS NULL OR EXISTS (
    ...
    SELECT b.*,
      (
        ...
        + CASE WHEN v_has_geo AND lower(public.immutable_unaccent(coalesce(b.city,''))) = lower(public.immutable_unaccent(coalesce(v_city_name,''))) THEN 60
               ...
               ELSE 0 END
        + CASE WHEN array_length(v_spec_ids,1) IS NOT NULL THEN 55 ELSE 0 END
...
GRANT EXECUTE ON FUNCTION public.search_therapists(text, text, text, int) TO anon, authenticated;
