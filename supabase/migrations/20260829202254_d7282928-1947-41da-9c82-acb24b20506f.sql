...
DECLARE
  ...
  v_contact_id uuid;
BEGIN
  -- Rattachement CRM pour tous les thérapeutes, quel que soit le plan.
  v_email := NULLIF(lower(trim(COALESCE(NEW.client_email, ''))), '');
  v_phone := NULLIF(regexp_replace(COALESCE(NEW.client_phone, ''), '[^0-9+]', '', 'g'), '');
  v_first := NULLIF(trim(split_part(COALESCE(NEW.client_name, ''), ' ', 1)), '');
  v_last := NULLIF(trim(regexp_replace(COALESCE(NEW.client_name, ''), '^\S+\s*', '')), '');
  ...
  IF v_contact_id IS NULL THEN
    ...
    IF v_contact_id IS NULL THEN
      INSERT INTO public.crm_client_contacts (
        therapist_id, first_name, last_name, email, phone, source, status, notes
      ) VALUES (
        NEW.therapist_id,
        COALESCE(v_first, ''),
        COALESCE(v_last, ''),
...
FOR EACH ROW
EXECUTE FUNCTION public.trg_crm_contact_from_appointment();