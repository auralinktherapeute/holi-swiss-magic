...
import type { TherapistMapProps, TherapistMapTherapist } from "./TherapistMap";

// Fix Leaflet default icon in Vite. This file is loaded only in the browser.
...
function safeInitials(therapist: TherapistMapTherapist): string {
  const a = (therapist.first_name?.[0] ?? "").toUpperCase();
  const b = (therapist.last_name?.[0] ?? "").toUpperCase();
  return (a + b) || "?";
}

function createMarkerIcon(therapist: TherapistMapTherapist, isSelected: boolean, resolvedPhoto?: string) {
...
export function TherapistMapInner({ therapists, selectedId, onSelect, lang }: TherapistMapProps) {
    ...
    <div className="relative h-full w-full rounded-2xl overflow-hidden">
      ...
      >
        ...
        {positioned.map((t) => (
          ...
          >
            <Popup closeButton={false} className="holiswiss-popup">
              <div
                style={{
                  background: "#1a1035",
