...
import { Instagram, Linkedin, Music2, Clock, CheckCircle2, Pencil, XCircle, Sparkles, MessageSquare, ListChecks, Lightbulb, Trash2 } from "lucide-react";
import {
...
const PILLAR_LABEL: Record<string, string> = {
  // Piliers en vigueur depuis le 01/08/2026 (voir .agents/product-marketing.md § 9)
  standard: "Le Standard",
  chaise_en_face: "La Chaise d'en face",
  concret_suisse: "Le Concret suisse",
  preuve: "La Preuve",
  // Anciens piliers — conservés pour l'affichage des propositions déjà en base
...
const TOPIC_STATUS_META: Record<string, { label: string; cls: string }> = {
...
function MarketingPage() {
  ...
  const fetchTopics = useServerFn(listMarketingTopics);
  const { data: topicsData } = useQuery({
    queryKey: ["marketing-topics"],
    queryFn: () => fetchTopics(),
  });
  const topics = (topicsData?.rows ?? []) as Topic[];
  const queued = topics.filter((t) => t.status === "en_attente").length;
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-6 flex gap-1 rounded-xl border border-white/10 bg-white/[0.03] p-1">
        ...
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition ${
            tab === "topics" ? "bg-gradient-to-r from-[#b86ef9] to-[#5cc8fa] text-white" : "text-white/60 hover:text-white"
          }`}
        >
          <Lightbulb className="h-4 w-4" /> Sujets
          {queued > 0 && (
      ...
      {tab === "topics" && <TopicsPanel topics={topics} />}
    </div>
  );
}
...
function TopicsPanel({ topics }: { topics: Topic[] }) {
    ...
    try {
      const res = await submit({
        data: {
          ...
          network: (network || undefined) as never,
      ...
      setSubject(""); setNetwork(""); setFormat(""); setDate("");
      ...
      <section>
        ...
        {topics.length === 0 ? (
          ...
          <ul className="space-y-3">
            {topics.map((t) => {
              const st = TOPIC_STATUS_META[t.status] ?? {
                label: t.status,
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-white/45">
                        Pour le {new Date(t.target_date).toLocaleDateString("fr-CH")}
                        {t.network && ` · ${NETWORK_META[t.network]?.label ?? t.network}`}
                        {t.format && ` · ${t.format}`}
                      </p>
                      {t.reject_reason && (
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`rounded-full border px-2 py-0.5 text-[11px] ${st.cls}`}>{st.label}</span>
                      {t.status === "en_attente" && (
                        <button
                          onClick={() => drop(t.id)}
