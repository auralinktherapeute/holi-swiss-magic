...
import { NEW_THERAPISTS_VARIANTS } from "@/components/holiswiss/NewTherapistsVariants";

/**
 * Mode comparaison des 3 versions de la section « Nouveaux thérapeutes ».
 * Purement présentationnel : données de démonstration, section publique inchangée.
 */
export const Route = createFileRoute("/preview/nouveaux-therapeutes")({
  ssr: false,
  component: PreviewNewTherapists,
});
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6">
        <header className="mb-6">
          <h1 className="text-2xl font-bold text-white sm:text-3xl">
            « Nouveaux thérapeutes » — 3 propositions
          </h1>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-[#d4c4e0]">
            Aperçus avec données de démonstration. La section actuelle du site n'est pas modifiée
            tant que vous n'avez pas choisi une version.
          </p>
        </header>
        ...
        {chosen && (
          ...
          >
            ...
            {NEW_THERAPISTS_VARIANTS.find((v) => v.id === chosen)?.name}. Rien n'a été appliqué au
            site — confirmez-moi « Applique la {chosen} ».
          </div>
        )}

        <div className="space-y-10">
          {NEW_THERAPISTS_VARIANTS.map(({ id, name, description, Component }) => (
            <section
              key={id}
