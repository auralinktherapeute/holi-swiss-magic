...
function Page() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getPublishedArticles({ data: { lang } }),
  });
              ...
              >
                ...
                <div className="aspect-video overflow-hidden bg-surface-alt">
                  {article.cover_image_url
                    ? <img src={article.cover_image_url} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                    : <div className="w-full h-full flex items-center justify-center text-muted-foreground/40 text-4xl">🌿</div>
                  }
                </div>
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="flex items-center gap-2 mb-3">
                    {article.category && (
                      <Badge variant="secondary" className="text-xs capitalize bg-primary/15 text-primary-light border-primary/20">
                        {article.category}
                      </Badge>
                    )}
                  ...
                  <h2 className="font-bold text-foreground text-lg leading-snug mb-2 group-hover:text-primary-light transition-colors">
                    {article.title}
                  </h2>
