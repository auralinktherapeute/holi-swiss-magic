...
import { HeartPulse, RefreshCw, Loader2, ExternalLink, CheckCircle2, AlertTriangle } from "lucide-react";
import {
  listHealthScores,
  getHealthDetail,
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => (
                  ...
                  >
                    <td className="p-2">
                      {r.name || "—"} <span className="text-white/40">· {r.canton}</span>
                    </td>
                    <td className="p-2">
          ...
          <div>
            {!selected ? (
              ...
              <DetailCard detail={detail} onChangeStatus={changeStatus} />
            )}
          </div>
        </div>
      )}
...
function DetailCard({ detail, onChangeStatus }: { detail: any; onChangeStatus: (id: string, s: string) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-[rgba(168,85,247,.25)] bg-[#2d1b4e]/80 p-6 backdrop-blur">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-white/45">
            {[t.address, t.city, t.canton].filter(Boolean).join(", ")}
          </p>
        ...
        {(["completude", "contenu", "activite", "visibilite"] as const).map((k) => (
          <div key={k} className="rounded-lg bg-white/5 p-2">
            <div className="font-semibold text-white">
              {s[`score_${k}`] ?? 0}<span className="text-white/40">/{CAT_MAX[k]}</span>
            </div>
          ...
          <ul className="space-y-1">
            {(s.strengths ?? []).map((x: any, i: number) => (
              <li key={i} className="flex items-start gap-1.5 text-sm text-white/80">
                <CheckCircle2 className="mt-0.5 shrink-0 text-green-400" size={14} /> {x.label}
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-1">
            {(s.gaps ?? []).map((x: any, i: number) => (
              <li key={i} className="flex items-start gap-1.5 text-sm text-white/80">
                <AlertTriangle className="mt-0.5 shrink-0 text-amber-400" size={14} /> {x.label}
              </li>
            ))}
      ...
      {s.article_idea && (
        <div className="mt-4 rounded-lg border border-cyan-400/30 bg-cyan-400/10 p-3 text-sm text-white/90">
          ✍️ <b>Idée d'article « Voix d'experts » :</b> {s.article_idea}
        </div>
      )}
    </div>
  );
