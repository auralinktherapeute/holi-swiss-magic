...
export const deleteMyAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    await notifyAdmin(
      "therapist_deleted",
      "Compte thérapeute supprimé (nLPD)",
        ...
        if (LOVABLE_API_KEY && RESEND_API_KEY) {
          await fetch("https://connector-gateway.lovable.dev/resend/emails", {
            ...
            body: JSON.stringify({
              ...
              html: `<p>Bonjour,</p><p>Votre compte HoliSwiss a été supprimé définitivement, conformément à votre demande et à la nLPD suisse (droit à l'effacement).</p><p>Toutes vos données personnelles ont été effacées de nos serveurs.</p><p>Merci pour la confiance que vous nous avez accordée.</p><p>— L'équipe HoliSwiss</p>`,
            }),
          });
        }
      } catch (e) {
