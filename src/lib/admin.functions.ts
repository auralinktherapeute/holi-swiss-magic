...
export const getAdminBadgeCounts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [therapists, waitlist, events, articles, pendingReviews, pendingReplies] = await Promise.all([
      ...
      (supabaseAdmin as any).from("reviews").select("id", { count: "exact", head: true }).eq("therapist_reply_status", "pending"),
    ]);
    return {
...
export const listWaitingListAdmin = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      .from("waiting_list")
      .select("id,email,created_at,source,status,first_name,last_name,phone,specialty,canton,message,invitation_status,invited_at,token_expires_at")
      .order("created_at", { ascending: false });
