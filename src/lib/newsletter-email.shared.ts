...
function paragraphs(text: string | null | undefined): string {
  ...
  return String(text)
    ...
    .map((block) => {
      ...
      <tr><td style="background:#f6f4ef;border-left:3px solid #7c3aed;border-radius:6px;padding:14px 16px;">
...
export function renderNewsletterEmail(input: NewsletterEmailInput): {
  ...
  const button =
    input.buttonLabel && input.buttonUrl
      ...
      <a href="${escapeHtml(input.buttonUrl)}" style="display:inline-block;padding:14px 26px;border-radius:999px;background:#7c3aed;color:#ffffff;font-weight:600;text-decoration:none;">${escapeHtml(input.buttonLabel)}</a>
    </p>`
      : "";
  ...
  const inner = `${preheader}
    <div style="color:#7c3aed;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;margin:0 0 10px;">La Lettre Holiswiss</div>
  ...
  const linkStyle = "color:#7c3aed;text-decoration:underline;";
  const footerExtra = `
